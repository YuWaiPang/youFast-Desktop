...
namespace youFast
{
    ...
    public class Presentation
    {
        public void presentationOfData(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, Dictionary<int, List<double>> ramDetailgz, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, ConcurrentDictionary<string, ConcurrentDictionary<int, int>> tableFact, Dictionary<string, Dictionary<int, Dictionary<double, string>>> remK2V, string sourceFolder, string db1Folder, Dictionary<decimal, Dictionary<string, StringBuilder>> screenControl, Dictionary<decimal, int> distinctDimensionChecksumList, List<decimal> distinctListChecksum, List<decimal> distinctSet, Dictionary<decimal, decimal> unsorted2SortedCheksum, List<int> distinctDimension, Dictionary<int, string> xyDimension, List<int> crosstabDimension, List<int> yDimension, Dictionary<int, Dictionary<double, string>> _ramKey2Valuegz, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz, List<int> revisedX, List<int> revisedY, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, StringBuilder debug, string outputFolder, ConcurrentDictionary<decimal, clientMachine.response> responseDict, Dictionary<int, Dictionary<double, string>> distinctRamKey2Value, Dictionary<int, Dictionary<double, double>> ramKey2Order, Dictionary<int, Dictionary<double, double>> ramOrder2Key,  Dictionary<int, List<double>> distinctList, Dictionary<int, StringBuilder> htmlTable, List<int> measure, char csvWriteSeparator, Dictionary<int, string> dataSortingOrder, int xAddressCol, int yAddressCol, int presentationJob, Dictionary<int, int> stopPresentationThread, ConcurrentDictionary<decimal, Thread> presentationThread, Dictionary<int, string> columnMoveOrder, int moveColumnID, string startRotateDimension, Dictionary<string, string> columnName2ID, string startMoveDimension, Dictionary<decimal, List<int>> distinctList2DrillDown)
        {
            ...
            if ((crosstabDimension.Count == 0 && distinctDimension.Count == 1) || crosstabDimension.Count == 0 && !columnMoveOrder.ContainsKey(moveColumnID) || (crosstabDimension.Count == 0 && columnMoveOrder.ContainsKey(moveColumnID) && columnMoveOrder[moveColumnID] != "Y2X") || (columnMoveOrder.ContainsKey(moveColumnID) && crosstabDimension.Count == 1 && columnMoveOrder[moveColumnID] == "X2Y"))
            {
                ...
                {
                    yDimension = distinctDimension;                  
                    chanagePresentation.sortSelectedYdimension(sortedYdimension, yDimension, distinctYramKey2Value, ramKey2Valuegz, revisedX, revisedY, requestID, clientSessionVariable, requestDict,  debug);
                }
                ...
                chanagePresentation.sortYdata(userPreference, distinctListChecksum, distinctSet, unsorted2SortedCheksum, sortedYdimension, requestID, clientSessionVariable, requestDict,  outputFolder, responseDict, distinctRamKey2Value, distinctYramKey2Value, YdistinctList, ramKey2Order, ramOrder2Key, yDimension, distinctList, revisedY, dataSortingOrder);

                if (requestDict[requestID].debugOutput == "Y")
                {
                    using (StreamWriter toDisk = new StreamWriter(outputFolder + userPreference["system"].slash + "debug" + userPreference["system"].slash + "debugInfo.txt"))
                    {
                        toDisk.Write(debug);
                        toDisk.Close();
                        debug.Clear();
                ...
                chanagePresentation.startPaginationInYM(isResponse, requestID2SessionID, wsServerSessionID, ramDetailgz, userPreference, tableFact, remK2V, sourceFolder, db1Folder, _ramKey2Valuegz, screenControl, distinctSet, unsorted2SortedCheksum, requestID, responseDict, distinctList, YdistinctList, clientSessionVariable, requestDict,  htmlTable, ramKey2Valuegz, distinctRamKey2Value, outputFolder, revisedY, measure, csvWriteSeparator, dataSortingOrder, presentationJob, presentationThread, dataSortingOrder, distinctList2DrillDown);
            }
            ...
            {                
                ...
                if (startMoveDimension == "Y")
                    chanagePresentation.sortSelectedDimensionOrder(distinctDimension, crosstabDimension, requestID, clientSessionVariable, requestDict,  debug, yDimension, xyDimension, revisedX, revisedY, columnMoveOrder);

                chanagePresentation.sortSelectedXYdimension(sortedXdimension, sortedYdimension, sortedXYdimension, sortedRevisedXY, crosstabDimension, distinctXramKey2Value, ramKey2Valuegz, yDimension, distinctYramKey2Value, revisedX, revisedY, requestID, clientSessionVariable, requestDict,  debug);
                ...
                var crosstabCell = Convert.ToDecimal(XdistinctList[0].Count) * Convert.ToDecimal(YdistinctList[0].Count) * Convert.ToDecimal(measure.Count);
               
                if (crosstabCell < userPreference["system"].maxCrosstabCell)
                {                   
                    chanagePresentation.crosstab(userPreference, YXMdistinctDrillKey, distinctDimensionChecksumList, crosstabAddress2DrillSetDict, sortedRevisedXY, sortedXdimension, sortedYdimension, csvWriteSeparator, outputFolder, requestID, responseDict, clientSessionVariable, requestDict,  measure, distinctList, XdistinctList, YdistinctList, distinctXramKey2Value, distinctYramKey2Value, YXMdistinctList, debug, xAddressCol, yAddressCol);
                    ...
                    for (int j = 0; j < crosstabDimension.Count; j++)                   
                        crosstab.Add(ramKey2Valuegz[crosstabDimension[j]][0].ToString());

                   // clientSessionVariable[requestDict[requestID].clientSession].clientSessionCrosstabName = crosstab;
                    clientSessionVariable[responseDict[requestID].serverSession].clientSessionCrosstabName = crosstab;

                    chanagePresentation.startPaginationInYXM(isResponse, requestID2SessionID, wsServerSessionID, ramDetailgz, tableFact, userPreference, screenControl, YXMdistinctDrillKey, distinctDimensionChecksumList, crosstabAddress2DrillSetDict, distinctSet, unsorted2SortedCheksum, distinctList2DrillDown, sortedXdimension, YXMdistinctList, clientSessionVariable, requestDict,  requestID, XdistinctList, YdistinctList, responseDict, htmlTable, _ramKey2Valuegz, ramKey2Valuegz, distinctXramKey2Value, distinctYramKey2Value, outputFolder, revisedY, measure, csvWriteSeparator, dataSortingOrder, presentationJob, stopPresentationThread, presentationThread);
                }
                else
                {
                    StringBuilder errorMessage = new StringBuilder();
                    errorMessage.AppendLine("Error Description");                    
                    errorMessage.AppendLine("\"Selected XY dimensions of the filtered list " + (char)39 + requestDict[requestID].importFile + (char)39 + " has calculated crosstab cells of " + string.Format("{0:#,0}", crosstabCell) + " which is greater than current user setting limit of " + string.Format("{0:#,0}", userPreference["system"].maxCrosstabCell) + ".\""); 
    ...
    public class ChangeInReportPresentation
    {
        ...
        {   
            ...
            // clientSessionVariable[requestDict[requestID].clientSession].clientSessionCrosstabName = null;
            clientSessionVariable[responseDict[requestID].serverSession].clientSessionCrosstabName = null;
        ...
        public void startPaginationInYM(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, Dictionary<int, List<double>> ramDetailgz, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, ConcurrentDictionary<string, ConcurrentDictionary<int, int>> tableFact, Dictionary<string, Dictionary<int, Dictionary<double, string>>> remK2V, string sourceFolder, string db1Folder, Dictionary<int, Dictionary<double, string>> _ramKey2Valuegz, Dictionary<decimal, Dictionary<string, StringBuilder>> screenControl, List<decimal> distinctSet, Dictionary<decimal, decimal> unsorted2SortedCheksum, decimal requestID, ConcurrentDictionary<decimal, clientMachine.response> responseDict,  Dictionary<int, List<double>> distinctList, Dictionary<int, List<double>> YdistinctList, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, Dictionary<int, StringBuilder> htmlTable, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz, Dictionary<int, Dictionary<double, string>> distinctRamKey2Value, string outputFolder, List<int> revisedY, List<int> measure, char csvWriteSeparator, Dictionary<int, string> sortingOrder, int presentationJob, ConcurrentDictionary<decimal, Thread> presentationThread, Dictionary<int, string> dataSortingOrder, Dictionary<decimal, List<int>> distinctList2DrillDown)
        {
            ...
            currentExport.ramDistinct2HtmlYMtable(isResponse, requestID2SessionID, wsServerSessionID, userPreference, tableFact, remK2V, sourceFolder, db1Folder, _ramKey2Valuegz, screenControl, Ypage, YtotalPage, YpageStart, YpageEnd, requestID, clientSessionVariable, requestDict,  responseDict, YdistinctList, distinctRamKey2Value, outputFolder, dataSortingOrder);
        
            FinalEvent currentLoop = new FinalEvent();
            currentLoop.reportEvent(isResponse, requestID2SessionID, wsServerSessionID, ramDetailgz, userPreference, tableFact, remK2V, sourceFolder, db1Folder, _ramKey2Valuegz, screenControl, distinctSet, requestID, clientSessionVariable, requestDict,  responseDict, YpageStart, YpageEnd, Yrow, Ypage, YtotalPage, YdistinctList, ramKey2Valuegz, distinctRamKey2Value, outputFolder, csvWriteSeparator, sortingOrder, presentationJob, presentationThread, distinctList2DrillDown);                   
        }

        // YXM route (crosstab)
        public void sortSelectedDimensionOrder(List<int> distinctDimension, List<int> crosstabDimension, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, StringBuilder debug, List<int> yDimension, Dictionary<int, string> xyDimension, List<int> revisedX, List<int> revisedY, Dictionary<int, string> columnMoveOrder)
        {
            int moveCol;
        ...
        public void startPaginationInYXM(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, Dictionary<int, List<double>> ramDetailgz, ConcurrentDictionary<string, ConcurrentDictionary<int, int>> tableFact, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, Dictionary<decimal, Dictionary<string, StringBuilder>> screenControl, Dictionary<int, List<double>> YXMdistinctDrillKey, Dictionary<decimal, int> distinctDimensionChecksumList, Dictionary<double, decimal> crosstabAddress2DrillSetDict, List<decimal> distinctSet, Dictionary<decimal, decimal> unsorted2SortedCheksum, Dictionary<decimal, List<int>> distinctList2DrillDown, List<int> sortedXdimension, Dictionary<int, List<double>> YXMdistinctList, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, decimal requestID, Dictionary<int, List<double>> XdistinctList, Dictionary<int, List<double>> YdistinctList, ConcurrentDictionary<decimal, clientMachine.response> responseDict, Dictionary<int, StringBuilder> htmlTable, Dictionary<int, Dictionary<double, string>> _ramKey2Valuegz, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz, Dictionary<int, Dictionary<double, string>> distinctXramKey2Value, Dictionary<int, Dictionary<double, string>> distinctYramKey2Value, string outputFolder, List<int> revisedY, List<int> measure, char csvWriteSeparator, Dictionary<int, string> sortingOrder, int presentationJob, Dictionary<int, int> stopPresentationThread, ConcurrentDictionary<decimal, Thread> presentationThread)
        {
            ...
            responseDict[requestID].endTime = DateTime.Now;   
            ExportHTML currentExport = new ExportHTML();
            currentExport.ramDistinct2HtmlCrosstabTable(isResponse, requestID2SessionID, wsServerSessionID, tableFact, userPreference, _ramKey2Valuegz, screenControl, YXMdistinctDrillKey, Xpage, XtotalPage, Ypage, YtotalPage, XpageStart, XpageEnd, yHeaderCol, YpageStart, YpageEnd, XdistinctList, requestID, clientSessionVariable, requestDict,  responseDict, YXMdistinctList,distinctXramKey2Value, distinctYramKey2Value, outputFolder, sortingOrder);            

            FinalEvent currentLoop = new FinalEvent();
            currentLoop.crosstabReportEvent(isResponse, requestID2SessionID, wsServerSessionID, ramDetailgz, tableFact, userPreference, screenControl, YXMdistinctDrillKey, crosstabAddress2DrillSetDict, distinctSet, unsorted2SortedCheksum, distinctYramKey2Value, distinctList2DrillDown, requestID, clientSessionVariable, requestDict,  responseDict, Xpage, XpageStart, XpageEnd, Xcolumn, yHeaderCol, YpageStart, YpageEnd, Yrow, Ypage, YtotalPage, XtotalPage, htmlTable, XdistinctList, YdistinctList, YXMdistinctList, _ramKey2Valuegz, ramKey2Valuegz, distinctXramKey2Value, distinctYramKey2Value, outputFolder, revisedY, measure, csvWriteSeparator, sortingOrder, presentationJob, stopPresentationThread, presentationThread);
