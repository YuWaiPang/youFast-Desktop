...
namespace youFast
{
    ...
    {
        public void ramdistint2displayFilterDropDownList(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, string outputFolder, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz)
        {            
            ...
            html.AppendLine("<button id=\"exe2\" class=\"w3-button w3-round-xlarge w3-green\" onclick=\"moveColumn()\">Move Column</button>");
            html.AppendLine("<button id=\"exe\" class=\"w3-button w3-round-xlarge w3-green\" onclick=\"drillRow()\">Drill Row</button>");
            html.AppendLine("</control>");
            html.AppendLine("</p>");

            requestID2SessionID[requestID].Send(html.ToString());           
            ...
            if (requestDict[requestID].debugOutput == "Y")
            {
                ...
                using (StreamWriter toDisk = new StreamWriter(outputFolder + userPreference["system"].slash + "debug" + userPreference["system"].slash + "displayFilterDropDownList.txt"))
                {
                    toDisk.Write(html);
                    toDisk.Close();
                }
        ...
        public void ramdistinct2AddFilter(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, string outputFolder, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, int serialID, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz)
        {   
            ...
            StringBuilder html = new StringBuilder();
            var addColumnName = requestDict[requestID].filterColumn.Replace("#", " ");
            ...
            if (requestDict[requestID].debugOutput == "Y")
            {
                ...
                using (StreamWriter toDisk = new StreamWriter(outputFolder + userPreference["system"].slash + "debug" + userPreference["system"].slash + "addFilterHTML.txt"))
                {
                    toDisk.Write(html);
                    toDisk.Close();
                }
        ...
        public void ramdistinct2DisplaySelectedFilterValue(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, string outputFolder, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, int serialID, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz, List<string> displayRandomSelectedValue)
        {
            List<string> sortedValue = new List<string>();
            Dictionary<string, string> columnName2ID = new Dictionary<string, string>();
            for (int i = 0; i < ramKey2Valuegz.Count; i++)
            ...
            responseDict[requestID].displaySelectedValueHtml = html;
        ...
        public void ramdistint2Crosstab(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, ConcurrentDictionary<string, ConcurrentDictionary<int, int>> tableFact, string outputFolder, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz)
        {            
            StringBuilder html = new StringBuilder();
            ...
            if (requestDict[requestID].debugOutput == "Y")
            {
                using (StreamWriter toDisk = new StreamWriter(outputFolder + userPreference["system"].slash + "debug" + userPreference["system"].slash + "addCrosstab.txt"))
                {
                    toDisk.Write(html);
                    toDisk.Close();
                }
        ...
        public void ramdistint2CrosstabUpdate(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, ConcurrentDictionary<string, ConcurrentDictionary<int, int>> tableFact, string outputFolder, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz)
        {
            StringBuilder html = new StringBuilder();
            html.AppendLine("<select id=\"addCrosstab\" class=\"w3-select w3-col m2 w3-border w3-hover-border-Blue\">");
            html.AppendLine("<option value=\"Add Crosstab Dimension\">Reorganise in X Direction</option>");
            ...
            {               
                if (ramKey2Valuegz[i].Count > 1)
                {
                    ...
                    if (clientSessionVariable[responseDict[requestID].serverSession].clientSessionCrosstabName != null)
                    {
                        if (clientSessionVariable[responseDict[requestID].serverSession].clientSessionCrosstabName.Contains(ramKey2Valuegz[i][0]))
        ...
        public void ramdistint2Measurement(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, string outputFolder, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz)
        {   
            StringBuilder html = new StringBuilder();
            ...
            for (int i = 0; i < ramKey2Valuegz.Count; i++)
            {
                if (ramKey2Valuegz[i].Count == 1)
                {
                    ...
                    if (clientSessionVariable[responseDict[requestID].serverSession].clientSessionMeasurementName != null)
                    {
                        if (clientSessionVariable[responseDict[requestID].serverSession].clientSessionMeasurementName.Contains(trimWhitespace))
            ...
            if (requestDict[requestID].debugOutput == "Y")
            {
                using (StreamWriter toDisk = new StreamWriter(outputFolder + userPreference["system"].slash + "debug" + userPreference["system"].slash + "addMeasurement.txt"))
                {
                    toDisk.Write(html);
                    toDisk.Close();
                }
        ...
        public void ramdistint2AllMeasurement(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, string outputFolder, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz)
        {
            StringBuilder html = new StringBuilder();
            html.AppendLine("<select id=\"addMeasurement\" <select class=\"w3-select w3-col m2 w3-border w3-hover-border-Blue\" name=\"changeType\" id=\"measureType\" onchange=\"changeMeasureType(this)\">");
            html.AppendLine("<option value=\"sum\">Sum On Measurement</option>");
        ...
        public void ramdistint2DisplaySelectedColumn(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, ConcurrentDictionary<string, ConcurrentDictionary<int, int>> tableFact, string outputFolder, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz, List<string> selectDistinctCol)
        {            
            StringBuilder html = new StringBuilder();                 
            ...
            if (requestDict[requestID].debugOutput == "Y")
            {
                ...
                using (StreamWriter toDisk = new StreamWriter(outputFolder + userPreference["system"].slash + "debug" + userPreference["system"].slash + "addDisplayColumn.txt"))
                {
                    toDisk.Write(html);
                    toDisk.Close();
                }
        ...
        public void ramdistint2DisplayAllColumn(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, string outputFolder, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz)
        {
            StringBuilder html = new StringBuilder();
            html.AppendLine("<select id=\"addDisplayColumn\" <select class=\"w3-select w3-col m2 w3-border w3-hover-border-Blue\" name=\"addCrosstab\" id=\"addSelection\" onchange=\"addDisplayColumn(this)\">");
            html.AppendLine("<option value=\"Add Distinct Dimension\">Find Unique Set</option>");
        ...
        public void ramDistinct2HtmlYMtable(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, ConcurrentDictionary<string, ConcurrentDictionary<int, int>> tableFact, Dictionary<string, Dictionary<int, Dictionary<double, string>>> remK2V, string sourceFolder, string db1Folder, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz, Dictionary<decimal, Dictionary<string, StringBuilder>> screenControl, int Ypage, double YtotalPage, int YpageStart, int YpageEnd, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict,  Dictionary<int, List<double>> distinctList, Dictionary<int, Dictionary<double, string>> distinctRamKey2Value, string outputFolder, Dictionary<int, string> dataSortingOrder)
        {
           ...
           // responseDict[requestID].updateCrosstab = false;            
            ...
            html.AppendLine("<output-report id=" + nextPageID + " (" + wsServerSessionID.Count + ")" + "        name=\"response\"" + "  #fast=" + requestDict[requestID].importFile + ">");
            html.AppendLine("<div class=\"w3-container\" id=submitForm value=" + status + ">"); 
            html.AppendLine("<button class=\"w3-btn w3-hide w3-white w3-round-large\" id=\"requestID\" type =\"button\"> Request: " + nextPageID + "</button>");
            ...
            {
                var fileNameLength = filePath.Length - 1 - filePath.LastIndexOf((char)charNumber);
                var fileName = filePath.Substring(filePath.LastIndexOf((char)charNumber) + 1, fileNameLength);
            ...
            if (userPreference["system"].saveTable2Disk == true)
            {
                for (int i = 0; i < fileList.Count; i++)
                {
                    ...
                    html.AppendLine("<td class=\"w3-right-align w3-text-red\" name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + "0" + "</td>");
                    html.AppendLine("<td class=\"w3-left-align w3-text-red\"  name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + fileList[i] + "</td>");
                ...
                {              
                    int lastOneMatch = filePath.Substring(0, filePath.LastIndexOf((char)charNumber) + 1).LastIndexOf((char)92);
                    int lastTwoMatch = filePath.Substring(0, filePath.LastIndexOf((char)charNumber) - 1).LastIndexOf((char)92);
                ...
                {  
                    ...
                    if(remK2V.ContainsKey(fileList[i].ToString()) && userPreference["system"].saveTable2Memory == true)
                    {
                        if(tableFact.ContainsKey(fileList[i].ToString()))
                        {                        
                            html.AppendLine("<td class=\"w3-right-align w3-text-blue\" name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + string.Format("{0:#,0}", tableFact[fileList[i].ToString()][8]).ToString() + "</td>"); 
                            html.AppendLine("<td class=\"w3-left-align w3-text-blue\"  name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + fileList[i].ToString() + "</td>");
                        }
                    }
                    else 
                    {
                        if (tableFact.ContainsKey(fileList[i].ToString()))
                            html.AppendLine("<td class=\"w3-right-align w3-text-green\"  name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + string.Format("{0:#,0}", tableFact[fileList[i].ToString()][8]).ToString() + "</td>");
                        else
                            html.AppendLine("<td class=\"w3-right-align w3-text-green\"  name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + "0" + "</td>");

                        html.AppendLine("<td class=\"w3-left-align w3-text-green\"  name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + fileList[i].ToString() + "</td>");
                    }
                    html.AppendLine("</tr>");            
                }
            }

            if (userPreference["system"].saveTable2Disk == false)
            {
                for (int i = 0; i < fileList.Count; i++)
                {
                    html.AppendLine("<tr class=\"w3-hover-pale-yellow\">");

                    if (userPreference["system"].saveTable2Memory == true)
                    {
                        if (tableFact.ContainsKey(fileList[i].ToString()))
                        {
                            html.AppendLine("<td class=\"w3-right-align w3-text-green\"  name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + string.Format("{0:#,0}", tableFact[fileList[i].ToString()][8]).ToString() + "</td>");
                        ...
                        {
                            html.AppendLine("<td class=\"w3-right-align w3-text-red\"  name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + "0" + "</td>");
                            html.AppendLine("<td class=\"w3-left-align w3-text-red\"  name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + fileList[i].ToString() + "</td>");
                        }
                    ...
                    {
                        if (tableFact.ContainsKey(fileList[i].ToString()))
                            html.AppendLine("<td class=\"w3-right-align w3-text-red\" name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + string.Format("{0:#,0}", tableFact[fileList[i].ToString()][8]).ToString() + "</td>");
                        else
                            html.AppendLine("<td class=\"w3-right-align w3-text-red\"  name = \"onclick\" onclick=\"selectFromFileTable(this)\"" + " " + "id=" + fileList[i].Replace(" ", "?") + " >" + "0" + "</td>");
            ...
            requestID2SessionID[requestID].Send(html.ToString());
            if (responseDict[requestID].displaySelectedValueHtml != null && responseDict[requestID].displaySelectedValueHtml.Length > 0)
            {
                requestID2SessionID[requestID].Send(responseDict[requestID].displaySelectedValueHtml.ToString());
                responseDict[requestID].displaySelectedValueHtml = null;
            }

            ExportHTML currentHTML = new ExportHTML();
            currentHTML.ramdistint2displayFilterDropDownList(isResponse, requestID2SessionID, wsServerSessionID, userPreference, outputFolder, requestID, clientSessionVariable, requestDict, responseDict, ramKey2Valuegz);                      
            currentHTML.selectionCriteriaUpdate(isResponse, requestID2SessionID, wsServerSessionID, userPreference, tableFact, ramKey2Valuegz, screenControl, requestID, clientSessionVariable, requestDict,  responseDict, outputFolder);
            ...
            if (requestDict[requestID].debugOutput == "Y")
            {
                ...
                using (StreamWriter toDisk = new StreamWriter(outputFolder + userPreference["system"].slash + "debug" + userPreference["system"].slash + requestID.ToString() + ".html"))
                {
        ...
        public void drillDown2HtmlYMtable(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, Dictionary<int, List<double>> ramDetailgz, List<int> _drillSet, int Ypage, double YtotalPage, int YpageStart, int YpageEnd, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, string outputFolder, List<decimal> distinctSet, Dictionary<decimal, List<int>> distinctList2DrillDown, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz)
        {
            StringBuilder html = new StringBuilder();
            decimal nextPageID;
            ...
            if (requestDict[requestID].debugOutput == "Y")
            {
                ...
                using (StreamWriter toDisk = new StreamWriter(outputFolder + userPreference["system"].slash + "debug" + userPreference["system"].slash + requestID.ToString() + "DrillDown.html"))
                {
                    toDisk.Write(responseDict[requestID].html);
                    toDisk.Close();
                }
        ...
        public void drillDown2HtmlYMtableCrosstab(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, Dictionary<int, List<double>> ramDetailgz, List<decimal> _drillSet, int Ypage, double YtotalPage, int YpageStart, int YpageEnd, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, string outputFolder, Dictionary<decimal, List<int>> distinctList2DrillDown, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz)
        {
            StringBuilder html = new StringBuilder();
            decimal nextPageID;
        ...
        public void selectionCriteriaUpdate(ConcurrentDictionary<string, bool> isResponse, ConcurrentDictionary<decimal, IWebSocketConnection> requestID2SessionID, List<IWebSocketConnection> wsServerSessionID, ConcurrentDictionary<string, clientMachine.userPreference> userPreference, ConcurrentDictionary<string, ConcurrentDictionary<int, int>> tableFact, Dictionary<int, Dictionary<double, string>> ramKey2Valuegz, Dictionary<decimal, Dictionary<string, StringBuilder>> screenControl, decimal requestID, ConcurrentDictionary<string, clientMachine.clientSession> clientSessionVariable, ConcurrentDictionary<decimal, clientMachine.request> requestDict, ConcurrentDictionary<decimal, clientMachine.response> responseDict, string outputFolder)
        {
            ExportHTML currentExport = new ExportHTML();
            currentExport.ramdistint2CrosstabUpdate(isResponse, requestID2SessionID, wsServerSessionID, userPreference, tableFact, outputFolder, requestID, clientSessionVariable, requestDict, responseDict, ramKey2Valuegz);               

            List<string> selectDistinctCol = new List<string>();
           
            for (int i = 0; i < requestDict[requestID].distinctDimension.Count; i++)
            {
                var revisedText = requestDict[requestID].distinctDimension[i].ToString().Replace(" ", "#");                
                selectDistinctCol.Add(revisedText);                
            }

            currentExport.ramdistint2DisplaySelectedColumn(isResponse, requestID2SessionID, wsServerSessionID, userPreference, tableFact, outputFolder, requestID, clientSessionVariable, requestDict, responseDict, ramKey2Valuegz, selectDistinctCol);

            if (requestDict[requestID].processID == "refreshSelectFileList") 
                currentExport.ramdistint2Measurement(isResponse, requestID2SessionID, wsServerSessionID, userPreference, outputFolder, requestID, clientSessionVariable, requestDict,  responseDict, ramKey2Valuegz);           
            ...
            if (!requestDict[requestID].importFile.Contains("BalanceTable"))
            {
                for (int i = 0; i < requestDict[requestID].column.Count; i++)
                {
                    if (!selectDistinctCol.Contains(requestDict[requestID].column[i])) // add filter column to distinct dimension
                    {
                        var revisedText = requestDict[requestID].column[i].ToString().Replace(" ", "#");                           
                        selectDistinctCol.Add(revisedText);
                    }
                }                    
            }
            ...
            if (requestDict[requestID].importFile.Contains("BalanceTable"))             
                currentExport.ramdistint2displayFilterDropDownList(isResponse, requestID2SessionID, wsServerSessionID, userPreference, outputFolder, requestID, clientSessionVariable, requestDict, responseDict, ramKey2Valuegz);                   
            ...
            for (int i = 0; i < requestDict[requestID].measurement.Count; i++)
            {
                var revisedText = requestDict[requestID].measurement[i].ToString().Replace(" ", "#");
                selectMeasureCol.Add(revisedText);
            }
